...
- This page uses XML code and the getXMLContent function which are adapted from the XPath tutorials 
  available on W3CSchools at https://www.w3schools.com/xml/xpath_examples.asp
    ...
    <title>Barry's Bargain Books :: Search Results</title>
...
<body>
    ...
    <main id="main">
        <section class="boc-white-bg">
            ...
            <h1 class="boc-mblue-bg boc-pad-1 boc-center">Search Results</h1>
            ...
            <?php 
                try {
                    ...
                    $stmt = $connection->prepare("SELECT * FROM view_search WHERE title like CONCAT('%', :search, '%') OR short_desc like CONCAT('%', :search1, '%') OR long_desc like CONCAT('%', :search2, '%') OR author_name like CONCAT('%', :search3, '%') OR biography like CONCAT('%', :search4, '%') OR genre like CONCAT('%', :search5, '%') ");

                    $search = isset($_GET['search']) ? trim($_GET['search']) : null;

                    $stmt->bindParam(':search', $search, PDO::PARAM_INT);
                    $stmt->bindParam(':search1', $search, PDO::PARAM_INT);
                    $stmt->bindParam(':search2', $search, PDO::PARAM_INT);
                    $stmt->bindParam(':search3', $search, PDO::PARAM_INT);
                    $stmt->bindParam(':search4', $search, PDO::PARAM_INT);
                    $stmt->bindParam(':search5', $search, PDO::PARAM_INT);
