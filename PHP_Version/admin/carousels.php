...
*/
    ...
    <title>Barry's Bargain Books:: Carousel Index</title>
    ...
    <main id="main">
    	<section class="breadcrumb">
            <div id="breadcrumb" class="boc-grid-container">
                ...
                <span class="current">Carousel Images</span>
            </div>
        </section>

        <h2 class="boc-center">Carousel Image Index</h2>

    	<div id="paged-list" class="boc-grid-container">
	    	<div class="form-row header">
				<span class="list_actions">Actions</span>
				<span class="list_name">Carousel Text</span>			
			</div>
      
        	<div class="form-row rollover">
        		<span class="list_name right"><a href="carousel_add.php" title="Add a new Carousel Image">Add a new Carousel Image</a></span>
			</div>
...
try {
  ...
  $stmt = $connection->prepare("SELECT COUNT(*) as count FROM carousel");
  $stmt->execute();
...
try {
	...
	$stmt = $connection->prepare("SELECT carouselID, description FROM carousel LIMIT :offset, :rowsperpage");
	...
	while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) { ?>
  		<div class="form-row rollover">
  			<span class="list_name"><?php echo $result['description']; ?></span>
  			<span class="list_edit icon"><a href="carousel_edit.php?id=<?php echo $result['carouselID']; ?>" title="edit this carousel image"><i class="fas fa-edit"></i></a></span>
  			<span class="list_delete icon"><a href="javascript:makeSureDelete('carousel_delete.php?id=<?php echo $result['carouselID']; ?>');" title="delete this carousel image"><i class="far fa-trash-alt"></i></i></a></span>
  		</div>
  <?php 
	}
} catch( PDOExecption $error ) {
