...
*/
    ...
    <title>Barry's Bargain Books:: Author Index</title>
...
<body>
    ...
    <main id="main">
        ...
        <h2 class="boc-center">Author Index</h2>

    	<div id="paged-list" class="boc-grid-container">
	    	<div class="form-row header">
				<span class="list_name">Author Name</span>
				<span class="list_actions">Actions</span>
			</div>

			<div class="form-row rollover">
			    <span class="list_name right"><a href="author_add.php" title="Add a new Author">Add a new Author</a></span>
			</div>
...
try {
  ...
  $stmt = $connection->prepare("SELECT COUNT(*) as count FROM authors");
  $stmt->execute();
...
try {
	...
	$stmt = $connection->prepare("SELECT authorID, CONCAT(firstname, ' ', surname) AS fullname FROM authors LIMIT :offset, :rowsperpage");
	...
	while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) { ?>
  		<div class="form-row rollover">
  			...
  			<span class="list_edit icon"><a href="author_edit.php?id=<?php echo $result['authorID']; ?>" title="edit this Author"><i class="fas fa-edit"></i></a></span>
  			<span class="list_delete icon"><a href="javascript:makeSureDelete('author_delete.php?id=<?php echo $result['authorID']; ?>');" title="delete this Author"><i class="far fa-trash-alt"></i></i></a></span>
  		</div>
  <?php 
	}
} catch( PDOExecption $error ) {
