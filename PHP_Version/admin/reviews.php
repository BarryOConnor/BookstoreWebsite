...
*/
    ...
    <title>Barry's Bargain Books:: Review Index</title>
...
<body>
    ...
    <main id="main">
        ...
        <h2 class="boc-center">Genre Index</h2>

    	<div id="paged-list" class="boc-grid-container">
	    	<div class="form-row header">
				<span class="list_name">Review Source</span>
				<span class="list_actions">Actions</span>
			</div>

			<div class="form-row rollover">
        		<span class="list_name right"><a href="review_add.php" title="Add a new Review">Add a new Review</a></span>
			</div>
...
try {
  ...
  $stmt = $connection->prepare("SELECT COUNT(*) as count FROM reviews");
  $stmt->execute();
...
try {
	...
	$stmt = $connection->prepare("SELECT reviewID, source, content FROM reviews LIMIT :offset, :rowsperpage");
	...
	while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) { ?>
  		<div class="form-row rollover">
  			<span class="list_name"><?php echo $result['source']." :: \"".substr($result['content'], 0, 100)."\""; ?></span>
  			<span class="list_edit icon"><a href="review_edit.php?id=<?php echo $result['reviewID']; ?>" title="edit this review"><i class="fas fa-edit"></i></a></span>
  			<span class="list_delete icon"><a href="javascript:makeSureDelete('review_delete.php?id=<?php echo $result['reviewID']; ?>');" title="delete this review"><i class="far fa-trash-alt"></i></i></a></span>
  		</div>
  <?php 
	}
} catch( PDOExecption $error ) {
