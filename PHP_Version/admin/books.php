...
*/
    ...
    <title>Barry's Bargain Books:: Book Index</title>
...
<body>
    ...
    <main id="main">
        ...
        <h2 class="boc-center">Book Index</h2>

    	<div id="paged-list" class="boc-grid-container">
	    	<div class="form-row header">
				<span class="list_actions">Actions</span>
				<span class="list_name">Book Name</span>			
			</div>
      
        	<div class="form-row rollover">
        		<span class="list_name right"><a href="book_add.php" title="Add a new Book">Add a new Book</a></span>
			</div>
...
try {
  ...
  $stmt = $connection->prepare("SELECT COUNT(*) as count FROM books");
  $stmt->execute();
...
try {
	...
	$stmt = $connection->prepare("SELECT bookID, title FROM books LIMIT :offset, :rowsperpage");
	...
	while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) { ?>
  		<div class="form-row rollover">
  			<span class="list_name"><?php echo $result['title']; ?></span>
  			<span class="list_edit icon"><a href="book_edit.php?id=<?php echo $result['bookID']; ?>" title="edit this book"><i class="fas fa-edit"></i></a></span>
  			<span class="list_delete icon"><a href="javascript:makeSureDelete('book_delete.php?id=<?php echo $result['bookID']; ?>');" title="delete this Book"><i class="far fa-trash-alt"></i></i></a></span>
  		</div>
  <?php 
	}
} catch( PDOExecption $error ) {
