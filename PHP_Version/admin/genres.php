...
*/
    ...
    <title>Barry's Bargain Books:: Genre Index</title>
    ...
    <main id="main">
    	...
    	<div id="paged-list" class="boc-grid-container">
	    	<div class="form-row header">
				...
				<span class="list_name">Genre Name</span>			
			</div>
      
        	<div class="form-row rollover">
        		<span class="list_name right"><a href="genre_add.php" title="Add a new genre">Add a new Genre</a></span>
			</div>
...
try {
  ...
  $stmt = $connection->prepare("SELECT COUNT(*) as count FROM genres");
  $stmt->execute();
...
try {
	...
	$stmt = $connection->prepare("SELECT genreID, name FROM genres LIMIT :offset, :rowsperpage");
	...
	while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) { ?>
  		<div class="form-row rollover">
  			<span class="list_name"><?php echo $result['name']; ?></span>
  			<span class="list_edit icon"><a href="genre_edit.php?id=<?php echo $result['genreID']; ?>" title="edit this genre"><i class="fas fa-edit"></i></a></span>
  			<span class="list_delete icon"><a href="javascript:makeSureDelete('genre_delete.php?id=<?php echo $result['genreID']; ?>');" title="delete this genre"><i class="far fa-trash-alt"></i></i></a></span>
  		</div>
  <?php 
	}
} catch( PDOExecption $error ) {
