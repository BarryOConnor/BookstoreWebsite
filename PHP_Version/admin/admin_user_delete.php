...
*/
    ...
    <title>Barry's Bargain Books:: Delete an Administrator</title>
    ...
    <main id="main">
    	<section class="breadcrumb">
            <div id="breadcrumb" class="boc-grid-container">
                ...
                <span class="separator">></span>
                <span class="current">Delete an Administrator</span>
            </div>
        </section>
<?php 
...
/**********************    delete the selected user    **********************/
	try {
	    // prepare and bind
	    $stmt = $connection->prepare("DELETE FROM admin_users WHERE adminID=:id;");
		...
		if($return_value) {   
            echo "<h2 class='boc-center'><i class='far fa-check-circle green'></i> Administrator Deleted</h2>";
...
/**********************    no id specified display an error    **********************/
 ...
 echo "<h2 class='boc-center'><i class='far fa-times-circle red'></i> An Administrator was not selected</h2>";
 echo "<p class='boc-center'>You will be returned to the Administrator Index in 3 seconds or you can <a href='admin_users.php'>return to the Administrator Index now</a></p>";
 header( "refresh:3;url=admin_users.php" );
} 
 ?>
