...
*/
    ...
    <title>Barry's Bargain Books:: User Index</title>
...
<body>
    ...
    <main id="main">
        ...
        <h2 class="boc-center">User Index</h2>
    	...
    	<div id="paged-list" class="boc-grid-container">
        	...
        	<div class="form-row rollover">
        		<span class="list_name right"><a href="user_add.php" title="Add a new genre">Add a new User</a></span>
			</div>
...
try {
	...
	$stmt = $connection->prepare("SELECT userID, name FROM users LIMIT :offset, :rowsperpage");
	...
	while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) { ?>
  		<div class="form-row rollover">
  			...
  			<span class="list_edit icon"><a href="user_edit.php?id=<?php echo $result['userID']; ?>" title="edit this user"><i class="fas fa-edit"></i></a></span>
  			<span class="list_delete icon"><a href="javascript:makeSureDelete('user_delete.php?id=<?php echo $result['userID']; ?>');" title="delete this user"><i class="far fa-trash-alt"></i></i></a></span>
  		</div>
  <?php 
	}
} catch( PDOExecption $error ) {
